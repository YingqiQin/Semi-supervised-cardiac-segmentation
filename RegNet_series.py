...
from MyDataSet import AllPhasesDataset_Pretrained
...
def train(args):
    ...
    train_dataset = AllPhasesDataset_Pretrained(data_dir=args.train_dir, all_data_csv=args.all_data_csv,
                                                val_data_csv=args.val_data_csv,
                                                image_size=args.image_size, mode='train')
    valid_dataset = AllPhasesDataset_Pretrained(data_dir=args.train_dir, all_data_csv=args.all_data_csv,
                                                val_data_csv=args.val_data_csv,
    ...
    for _iter in range(1, args.n_iter + 1):
        ...
        model.train()
        stn.train()
        for _step, (sup_images, sup_labels, unlabeled_img_list) in enumerate(train_bar):
        ...
        stn.eval()
        with torch.no_grad():
            for val_labed_img, val_unsup_img_list in valid_loader:
                length = len(val_unsup_img_list)
                sim_loss_valid_mid = 0.0
                grad_loss_valid_mid = 0.0
                loss_valid_mid = 0.0
                for index in range(length):
                    val_unsup_img = val_unsup_img_list[index]
                    # img & label
                    img_fixed = val_unsup_img.cuda()
                    img_moving = val_labed_img.cuda()
                    ...
                    sim_loss_valid_mid += sim_loss.item()
                    grad_loss_valid_mid += grad_loss.item()
                    loss_valid_mid += loss.item()
                sim_loss_valid_mid /= length
                grad_loss_valid_mid /= length
                loss_valid_mid /= length

                sim_loss_valid += sim_loss_valid_mid
                grad_loss_valid += grad_loss_valid_mid
                loss_valid += loss_valid_mid
