...
def training(args):
    ...
    for epoch in range(epochs):
        ...
        for step, (unsup_images, sup_images, sup_labels) in enumerate(train_bar):
            ...
            output_for_unsup = torch.cat([SegNet1(images.cuda()) for images in unsup_seg_images_list], dim=0)
            ce_loss = F.cross_entropy(output_for_unsup, warped_labels_for_seg.long().cuda())
            loss_weakly = ce_loss
            loss_weakly.requires_grad_(True)
            ...
            if epoch > 50:
                unsup_loss = F.l1_loss(unsup_pseudo_labels.float().cuda(), warped_labels_for_seg.float().cuda())
            else:
                unsup_loss = 0
            ...
            weakly_loss_train += loss_weakly
            sup_loss_train += sup_loss.item()
            seg_loss_train += sup_loss.item() + loss_weakly
