...
def test_reg(args):
    ...
    with torch.no_grad():
        for index in range(0, len(data_list['image_filenames']), 2):
            ...
            metrics_1.append(calculate_metrics_persubj(label_warped.cpu().detach().numpy() == 1,
                                                       tensor_fixed_gt.cpu().detach().numpy() == 1))
            metrics_2.append(calculate_metrics_persubj(label_warped.cpu().detach().numpy() == 2,
                                                       tensor_fixed_gt.cpu().detach().numpy() == 2))
            metrics_3.append(calculate_metrics_persubj(label_warped.cpu().detach().numpy() == 3,
                                                       tensor_fixed_gt.cpu().detach().numpy() == 3))
    ...
    with open(os.path.join(pred_dir, 'results.csv'), mode='w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=['image_filenames', 'dsc1', 'dsc2', 'dsc3', 'assd1', 'assd2', 'assd3',
                                               'hd95_1', 'hd95_2', 'hd95_3'])
        writer.writeheader()
