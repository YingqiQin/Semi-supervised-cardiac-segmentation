...
def training(args):
    ...
    Tperceiver = TPer(1, 3, mode='trained').cuda()
    ...
    optimizer_reg = optim.SGD(params_reg, lr=learning_rate, momentum=0.9, weight_decay=0.0001)
    optimizer_1 = optim.SGD(params_seg_1, lr=learning_rate, momentum=0.9, weight_decay=0.0001)
    optimizer_2 = optim.SGD(params_seg_2, lr=learning_rate, momentum=0.9, weight_decay=0.0001)
    ...
    for epoch in range(epochs):
        ...
        for step, (unsup_images, sup_images, sup_labels) in enumerate(train_bar):
            ...
            tp_loss = feature_sim_loss(Tperceiver([img_fixed.cuda(), img_moving.cuda()]),
