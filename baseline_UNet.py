...
def training(args):
    ...
    optimizer = optim.SGD(params1, lr=learning_rate, momentum=0.9, weight_decay=0.0001)
    best_valid_dsc = 0.0
    final_train_dsc = 0.0
    ...
    with torch.no_grad():
        for index in range(len(data_list['image_filenames'])):
            ...
            for i in range(img.shape[2]):
                tmp_image = torch.from_numpy(img[:, :, i]).unsqueeze(dim=0).unsqueeze(dim=0)
                outputs = net(tmp_image.to(device))
                tmp_prob = F.softmax(outputs + torch.clamp(torch.randn_like(outputs) * 0.1, -0.1, 0.1), dim=1)
                pred_res[:, :, i] = torch.argmax(tmp_prob[0, :, :, :], dim=0)
