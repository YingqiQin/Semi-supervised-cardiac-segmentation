...
def train():
    ...
    for _iter in range(1, args.n_iter + 1):
        ...
        train_bar = tqdm(range(int(len(train_list_permuted) / args.batch_size + 0.5)))
        for _step in train_bar:
            ...
            for _batch in range(min(args.batch_size, len(train_list_permuted))):
                subject = _step * args.batch_size + _batch
                img_fixed[_batch, 0, :, :, :] = train_imgs[0, subject, :, :, :]
                img_moving[_batch, 0, :, :, :] = train_imgs[1, subject, :, :, :]
        ...
        sim_loss_train /= int(len(train_list_permuted) / args.batch_size + 0.5)
        grad_loss_train /= int(len(train_list_permuted) / args.batch_size + 0.5)
        loss_train /= int(len(train_list_permuted) / args.batch_size + 0.5)
        ...
        with torch.no_grad():
            ...
            for _num in range(len(valid_list)):
                # img & label
                img_fixed[0, 0, :, :, :] = train_imgs[0, _num, :, :, :]
                img_moving[0, 0, :, :, :] = train_imgs[1, _num, :, :, :]
                label_fixed[0, 0, :, :, :] = train_labels[0, _num, :, :, :]
                label_moving[0, 0, :, :, :] = train_labels[1, _num, :, :, :]
                img_fixed = img_fixed.cuda()
                img_moving = img_moving.cuda()
