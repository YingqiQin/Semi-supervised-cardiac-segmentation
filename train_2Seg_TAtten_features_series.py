...
from MyDataSet import AllPhasesDataset
...
def training(args):
    ...
    train_dataset = AllPhasesDataset(data_dir=args.train_data_dir, all_data_csv=args.all_data_csv,
                                     val_data_csv=args.val_data_csv,
                                     image_size=args.image_size, mode='train')
    valid_dataset = AllPhasesDataset(data_dir=args.train_data_dir, all_data_csv=args.all_data_csv,
                                     val_data_csv=args.val_data_csv,
    ...
    feature_sim_loss = compute_mse

    Tperceiver = TAtten(1, 3, mode='trained').cuda()
    if args.TPer_state_dict_resume is not None:
        Tperceiver.load_state_dict(torch.load(args.TPer_state_dict_resume, map_location=device))
    else:
    ...
    for epoch in range(epochs):
        ...
        for step, (sup_images, sup_labels, unlabeled_img_list) in enumerate(train_bar):
            length = len(unlabeled_img_list)
            sim_loss_train_mid = 0.0
            grad_loss_train_mid = 0.0
            reg_loss_train_mid = 0.0
            av_crf_loss_mid = 0.0
            weakly_loss_train_mid = 0.0
            sup_loss_train_mid = 0.0
            seg_loss_train_mid = 0.0
            total_loss_train_mid = 0.0
            avdsc_train_mid = 0.0
            unsup_loss_train_mid = 0.0
            tp_loss_train_mid = 0.0
            for index in range(len(unlabeled_img_list)):
                unsup_images = unlabeled_img_list[index]
                ...
                for p in SegNet2.parameters():
                    p.requires_grad = False
                for p in Tperceiver.parameters():
                    p.requires_grad = False
                ...
                tp_loss = 10000 * feature_sim_loss(Tperceiver([img_fixed.cuda(), img_moving.cuda()]),
                                                   Tperceiver([img_warped, img_moving.cuda()]))
                reg_loss = sim_loss + args.alpha * grad_loss + unsup_loss + tp_loss
                ...
                sim_loss_train_mid += sim_loss.item()
                grad_loss_train_mid += grad_loss.item()
                reg_loss_train_mid += reg_loss.item()
                av_crf_loss_mid += crf_loss
                weakly_loss_train_mid += loss_weakly.item()
                sup_loss_train_mid += sup_loss.item()
                seg_loss_train_mid += sup_loss.item() + loss_weakly.item()
                total_loss_train_mid += loss.item()
                avdsc_train_mid += dsc.item()
                unsup_loss_train_mid += unsup_loss
                tp_loss_train_mid += tp_loss.item()
            sim_loss_train_mid /= length
            grad_loss_train_mid /= length
            reg_loss_train_mid /= length
            av_crf_loss_mid /= length
            weakly_loss_train_mid /= length
            sup_loss_train_mid /= length
            seg_loss_train_mid /= length
            total_loss_train_mid /= length
            avdsc_train_mid /= length
            unsup_loss_train_mid /= length
            tp_loss_train_mid /= length

            sim_loss_train += sim_loss_train_mid
            grad_loss_train += grad_loss_train_mid
            reg_loss_train += reg_loss_train_mid
            av_crf_loss += av_crf_loss_mid
            weakly_loss_train += weakly_loss_train_mid
            sup_loss_train += sup_loss_train_mid
            seg_loss_train += seg_loss_train_mid
            total_loss_train += total_loss_train_mid
            avdsc_train += avdsc_train_mid
            unsup_loss_train += unsup_loss_train_mid
            tp_loss_train += tp_loss_train_mid
        RegModel.eval()
        stn.eval()
        SegNet1.eval()
        SegNet2.eval()
        ...
        tp_loss_train /= len(train_loader)

        print(
            "Epoch[{}/{}] Training average total loss:{:.4f}, DSC:{:.3f}, Registration loss:{:.4f}, "
            "sup_loss:{:.3f}, weakly_loss:{:.3f}, unsup_loss:{:.4f}, tp_loss:{:.8f}, Validation average loss:{:.3f}, "
            "Validation "
            "average DCS:{:.3f}".format(
                epoch + 1, epochs,
                total_loss_train, avdsc_train,
                reg_loss_train, sup_loss_train, weakly_loss_train, unsup_loss_train, tp_loss_train,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--all_data_csv", type=str, default=None)
    parser.add_argument("--val_data_csv", type=str, default=None)
    ...
    parser.add_argument("--alpha", type=float, help="regularization parameter", dest="alpha",
                        default=0.01)
    parser.add_argument("--TPer_state_dict_resume", type=str, default=None)
